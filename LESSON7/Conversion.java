package LESSON7;
		...
		public Conversion() {
			...
			btnConvert.addActionListener(
					evt->{
					   ...
					   if(sMile != null && !sMile.isEmpty() && !isNumber(sMile))JOptionPane.showMessageDialog(null, "Error: Mile must be number. Check for your input.  ");
					   else {
						   	if (sMile.isEmpty()) dMile=0;
					        else  dMile= Double.parseDouble(sMile);
					   		}
					   if( sPound != null && !sPound.isEmpty() && !isNumber(sPound))JOptionPane.showMessageDialog(null, "Error: Pound must be number. Check for your input.  ");
					   else  {
						   if (sPound.isEmpty()) dPound=0;
						   else dPound= Double.parseDouble(sPound);
					   }
					   if( sGallon != null && !sGallon.isEmpty() && !isNumber(sGallon))JOptionPane.showMessageDialog(null, "Error: Gallon must be number. Check for your input.  ");
					   else { if(sGallon.isEmpty()) dGallon=0;
					          else    dGallon= Double.parseDouble(sGallon);
					   }
					   
					   if(sFahrenheit!= null && !sFahrenheit.isEmpty() &&  !isNumber(sFahrenheit))JOptionPane.showMessageDialog(null, "Error: Fahrenheit must be number. Check for your input.  "); 
					   else { if(sFahrenheit.isEmpty()) dFahrenheit=0;
					   			else dFahrenheit= Double.parseDouble(sFahrenheit);
					   	}
					   
					   if(sKilometer != null && !sKilometer.isEmpty() && !isNumber(sKilometer))JOptionPane.showMessageDialog(null, "Error: Kilometer must be number. Check for your input.  ");
					   else{ if(sKilometer.isEmpty()) dKilometer =0; 
					   else  dKilometer= Double.parseDouble(sKilometer);
					   
					   }
						   if(sKilogram != null && !sKilogram.isEmpty() &&  !isNumber(sKilogram))JOptionPane.showMessageDialog(null, "Error: Kilogram must be number. Check for your input.  ");
					   else  { if(sKilogram.isEmpty()) dKilogram =0; 
					   			else dKilogram= Double.parseDouble(sKilogram);
					   
					   }
					   if(sLitre != null && !sLitre.isEmpty() &&  !isNumber(sLitre))JOptionPane.showMessageDialog(null, "Error: Litre must be number. Check for your input.  ");
					   else { if (sLitre.isEmpty()) dLitre=0;
					   else dLitre= Double.parseDouble(sLitre);
					   }
					   
					   if(sCentigrade != null && !sCentigrade.isEmpty() && !isNumber(sCentigrade))JOptionPane.showMessageDialog(null, "Error: Centigrade must be number. Check for your input.  ");
					   else   { if (sCentigrade.isEmpty()) dCentigrade=0;
					   else dCentigrade= Double.parseDouble(sCentigrade);
					   }
					   
					// check for negative value.
						
						if(dMile<0 || dPound<0 || dGallon<0 ||  dKilometer<0||dKilogram<0||dLitre<0 ) {
							JOptionPane.showMessageDialog(null, "Error: Input must be non-negative number. Please check your field ");
							
						}
						
						
					   if(dMile==0 && dKilometer>0) dMile = 0.62 * dKilometer;
					   else if(dMile >0 && dKilometer==0)  dKilometer= 1.6* dMile;
					   else {   
						   dMile = 0.62 * dKilometer;
						   dKilometer= 1.6* dMile;
					   }
					  
					   if(dPound==0 && dKilogram >0) dPound =2.2 * dKilogram;
					   else if(dPound >0 && dKilogram==0) dKilogram = 0.45 *dPound ; 
					    
					    
					   if (dGallon==0 && dLitre>0) dGallon = 0.264 *dLitre ;
					   
					   else if(dGallon>0 && dLitre==0) dLitre= 3.785* dGallon;
	                    
					   if (dFahrenheit==0 && dCentigrade>0)  dFahrenheit= (dCentigrade*1.8) +32;
					   
					   else if(dFahrenheit>0 && dCentigrade==0)  dCentigrade= (dFahrenheit-32)/1.8;  
					   else {
					   dFahrenheit= (dCentigrade*1.8) +32;
					   dCentigrade	= (dFahrenheit-32)/1.8;   
					   }
					  // change double back to string and display with just two decimal places. See helper method for the details  
	...
	/**
	 * Check if the the input is a number or something else  
	 * @param input
